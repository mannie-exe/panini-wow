...
local function GetConfig()
	return PaniniWoW_Config or {}
end

local function SafeSetCVar(name, value)
	PaniniWoW.SafeSetCVar(name, value)
end

local function SafeGetCVar(name)
	return PaniniWoW.SafeGetCVar(name)
end

local function EnablePanini()
	PaniniWoW.EnablePanini()
end

local function DisablePanini()
	PaniniWoW.DisablePanini()
end
...
-- hooksecurefunc is 3.3.5a only; 1.12.1 falls back to direct monkey-patch
if hooksecurefunc then
	hooksecurefunc("ToggleGameMenu", function()
		if PaniniSettingsDialog then
			PaniniSettingsDialog:Hide()
		end
	end)
else
	local _orig_ToggleGameMenu = ToggleGameMenu
	ToggleGameMenu = function()
		if PaniniSettingsDialog and PaniniSettingsDialog:IsShown() then
			PaniniSettingsDialog:Hide()
		end
		_orig_ToggleGameMenu()
	end
end
...
aboutUrl:SetText("https://github.com/mannie-exe/panini-wow")
...
aboutUrl:SetScript("OnTextChanged", function()
	this:SetText("https://github.com/mannie-exe/panini-wow")
	this:HighlightText()
end)
...
local function CreateTabButton(parent, name, label, pageFrame, selected)
	...
	local fs2 = tab:GetFontString()
	if selected then
		tab.selected = true
		if fs2 then fs2:SetTextColor(1, 0.85, 0) end
	else
		if fs2 then fs2:SetTextColor(0.6, 0.6, 0.6) end
	end
...
-- Switch the active settings tab. Hides all pages, shows the selected one.
...
-- Sync dialog controls to current PaniniWoW_Config and CVar readback.
...
-- Reset dialog position to screen center (used by /panini reset ui).
function PaniniWoW.ResetDialogPosition()
	dialog:ClearAllPoints()
	dialog:SetPoint("CENTER", UIParent, "CENTER")
end

-- Toggle settings dialog visibility.
function PaniniWoW.ToggleSettings()
