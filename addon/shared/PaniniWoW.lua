-- PaniniWoW: configuration addon for PaniniWoW.dll.
...
PaniniWoW = PaniniWoW or {}
PaniniWoW.defaults = defaults

local frame = CreateFrame("Frame", "PaniniWoWFrame", UIParent)
...
local function SyncCVarsToDLL()
	...
	for key, spec in pairs(cvarMap) do
		if key ~= "enabled" then
			local val = c[key]
			if val == nil then
				val = defaults[key]
...
PaniniWoW.SafeSetCVar = SafeSetCVar
PaniniWoW.SafeGetCVar = SafeGetCVar
PaniniWoW.EnablePanini = EnablePanini
PaniniWoW.DisablePanini = DisablePanini
PaniniWoW.ToggleSettings = function() end

frame:SetScript("OnEvent", function()
	if event == "ADDON_LOADED" and (arg1 == "PaniniWoW-Classic" or arg1 == "PaniniWoW-WotLK") then
		if not PaniniWoW_Config then
			PaniniWoW_Config = {}
		end
		for k, v in pairs(defaults) do
			if PaniniWoW_Config[k] == nil then
				PaniniWoW_Config[k] = v
			end
		end
		SyncCVarsToDLL()
		if dllLoaded then
...
local function ResetSettings()
	local savedPos = PaniniWoW_Config.minimapPos
	PaniniWoW_Config = {}
	for k, v in pairs(defaults) do
		PaniniWoW_Config[k] = v
	end
	PaniniWoW_Config.minimapPos = savedPos
	SyncCVarsToDLL()
...
PaniniWoW.ResetSettings = ResetSettings

SLASH_PANINI1 = "/panini"
SLASH_PANINI2 = "/pw"

-- Slash command handler: /panini or /pw.
-- Subcommands: settings, toggle, on, off, strength, vertical, fill, fov,
-- fxaa, sharpen, debug, reset, status, cvars, debuginfo, help.
local function PaniniSlashHandler(msg)
	local args = {}
	...
	elseif cmd == "reset" then
		if val == "ui" then
			if PaniniWoW.ResetDialogPosition then
				PaniniWoW.ResetDialogPosition()
			end
...
SlashCmdList["PANINI"] = function(msg)
	local ok, err = pcall(PaniniSlashHandler, msg)
	if not ok then
		DEFAULT_CHAT_FRAME:AddMessage("|cffff0000[PaniniWoW] slash error: " .. tostring(err) .. "|r")
	end
end
